...
static int get_duration(struct bat *bat)
{
	...
	if (duration_i < 0 || duration_i > MAX_FRAMES)
		return -EINVAL;
...
static void test_loopback(struct bat *bat)
{
	...
	if (*thread_result_playback != 0) {
		loge(E_MSG_EXITTHREADP, "%d", *thread_result_playback);
		pthread_cancel(capture_id);
		exit(EXIT_FAILURE);
	} else {
		fprintf(bat->log, "Playback completed.\n");
	}
	...
	if (*thread_result_capture != 0) {
		loge(E_MSG_EXITTHREADC, "%d", *thread_result_capture);
		exit(EXIT_FAILURE);
	} else {
		fprintf(bat->log, "Capture completed.\n");
	}
}
...
static void test_playback(struct bat *bat)
{
	...
	/* check playback status */
	if (*thread_result != 0) {
		loge(E_MSG_EXITTHREADP, "%d", *thread_result);
		exit(EXIT_FAILURE);
	} else {
		fprintf(bat->log, "Playback completed.\n");
	}
...
static void test_capture(struct bat *bat)
{
	...
	/* check playback status */
	if (*thread_result != 0) {
		loge(E_MSG_EXITTHREADC, "%d", *thread_result);
		exit(EXIT_FAILURE);
	} else {
		fprintf(bat->log, "Capture completed.\n");
	}
...
static int bat_init(struct bat *bat)
{
	...
	if (bat->logarg) {
		...
		if (bat->log == NULL) {
			loge(E_MSG_OPENFILEC, "%s %d", bat->logarg, -errno);
			return -errno;
		}
	}
