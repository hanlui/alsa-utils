...
int read_wav_header(struct bat *bat, char *file, FILE *fp, bool skip)
{
	...
	if (ret != 1) {
		loge(E_MSG_READFILE, "header of %s:%zd", file, ret);
		return -EIO;
	}
	if ((riff_wave_header.magic != WAV_RIFF)
			|| (riff_wave_header.type != WAV_WAVE)) {
		loge(E_MSG_FILECONTENT, "%s is not a riff/wave file", file);
		return -EINVAL;
	}
	...
	do {
		...
		if (ret != 1) {
			loge(E_MSG_READFILE, "chunk of %s:%zd", file, ret);
			return -EIO;
		}
		...
		case WAV_FMT:
			...
			if (ret != 1) {
				loge(E_MSG_READFILE, "chunk fmt of %s:%zd",
						file, ret);
				return -EIO;
			}
			...
			if (skip == false) {
				...
				case 4:
					break;
				default:
					loge(E_MSG_PARAMS MSG_PCMFORMAT,
							"size=%d",
							bat->sample_size);
					return -EINVAL;
				}
		...
		default:
			...
			if (ret == -1) {
				loge(E_MSG_SEEKFILE, "unknown chunk of %s:%zd",
						file, ret);
				return -EINVAL;
			}
		}
